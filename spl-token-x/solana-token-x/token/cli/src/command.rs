...
use {
    crate::MAX_PRIO_FEE_MICRO_LAMPORTS as MAX_PRIO_FEE_MICRO_LAMPORTS_UNSAFE,
    crate::{
        bench::*,
        clap_app::*,
...
fn get_max_prio_fee_micro_lamports() -> Option<f64> {
    println!("MAX_PRIO_FEE_MICRO_LAMPORTS: {:?}", unsafe {
        MAX_PRIO_FEE_MICRO_LAMPORTS_UNSAFE
    });
    unsafe { MAX_PRIO_FEE_MICRO_LAMPORTS_UNSAFE }
}

lazy_static::lazy_static! {
    pub static ref MAX_PRIO_FEE_MICRO_LAMPORTS: Option<f64> = get_max_prio_fee_micro_lamports();
}
...
async fn command_create_account(
    ...
    let res = if is_associated {
        token
            .create_associated_token_account(&owner, *MAX_PRIO_FEE_MICRO_LAMPORTS)
            .await
    } else {
    ...
    let token = if !config.sign_only {
        ...
        if let Ok(extension) = source_state.get_extension::<TransferFeeAmount>() {
            if u64::from(extension.withheld_amount) != 0 {
                let res = token
                    .harvest_withheld_tokens_to_mint(&[&account], *MAX_PRIO_FEE_MICRO_LAMPORTS)
                    .await?;
                let tx_return = finish_tx(config, &res, false).await?;
...
async fn command_sync_native(config: &Config<'_>, native_account_address: Pubkey) -> CommandResult {
    ...
    let res = token
        .sync_native(&native_account_address, *MAX_PRIO_FEE_MICRO_LAMPORTS)
        .await?;
    let tx_return = finish_tx(config, &res, false).await?;
    Ok(match tx_return {
        TransactionReturnData::CliSignature(signature) => {
